...
test("@claim:release-manifest release metadata names every installer and its SHA-256", async () => {
  ...
  await mkdir(assets);
  const fixtures = [
    "Reader.Sideload.Library_0.1.4_aarch64.dmg",
    "Reader.Sideload.Library_0.1.4_x64.dmg",
    "Reader.Sideload.Library_0.1.4_x64.msi",
    "Reader.Sideload.Library_0.1.4_x64-setup.exe",
    "Reader.Sideload.Library_0.1.4_amd64.AppImage",
    "Reader.Sideload.Library_0.1.4_amd64.deb",
    "Reader.Sideload.Library-0.1.4-1.x86_64.rpm"
  ];
  for (const name of fixtures) await writeFile(resolve(assets, name), `fixture:${name}`);
  const result = spawnSync(process.execPath, [resolve("scripts/release-manifest.mjs"), "v0.1.4", assets], {
    cwd: directory,
